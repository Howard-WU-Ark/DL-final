    ...
    def forward(self, data_dict, **kwargs):
        ...
        for i in range(self.gnn_layer):
            ...
            if i == self.gnn_layer - 2:
                ...
                '''
                #没这项cross_graph_edge = getattr(self, 'cross_graph_edge_{}'.format(i))
                emb_edge1 = emb_edge1.transpose((0, 3, 1, 2))
                emb_edge2 = emb_edge2.transpose((0, 3, 1, 2))
                s = s.unsqueeze(1)
                new_emb_edge1 = cross_graph_edge(paddle.concat((emb_edge1, paddle.matmul(paddle.matmul(s, emb_edge2), s.transpose((0,1,3,2)))), axis=1).transpose((0, 2, 3, 1)))
                new_emb_edge2 = cross_graph_edge(paddle.concat((emb_edge2, paddle.matmul(paddle.matmul(s.transpose((0,1,3,2)), emb_edge1), s)), axis=1).transpose((0, 2, 3, 1)))
